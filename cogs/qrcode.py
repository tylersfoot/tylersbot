...
class Qrcode(commands.Cog):
    ...
    @commands.slash_command(name="qr", description="Generates a qr code from the given text/link.", integration_types={discord.IntegrationType.guild_install, discord.IntegrationType.user_install})
    async def qr(self, ctx, *, message: str):
        ...
        try:
            ...
            await ctx.respond('The text is too large to be converted into a QR code.', ephemeral=True)
            return
