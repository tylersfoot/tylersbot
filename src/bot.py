...
from core.logger import logger_initialize, log_info, log_error
from config.config import DEVELOPER_GUILD_ID, DEVELOPER_USER_IDS
from core.paths import TEMP_PATH, COGS_PATH
...
if __name__ == "__main__":
    ...
    async def change_status():
        ...
        next_status = next(status_cycle).format(
    ...
    dev_group = bot.create_group(
        ...
        default_member_permissions=discord.Permissions(
            permissions=0, administrator=True
        ),
    )

    @bot_group.command(
        name="uptime", description="Sends the bot's uptime since last restart."
    )
    ...
    @dev_group.command(
        name="reload", description="Loads/reloads cogs.", guild_ids=[962179884627669062]
    )
    async def bot_reload(ctx, extension: str):
        ...
        if extension == "all":
            ...
            for file in os.listdir(COGS_PATH):
                if file.endswith(".py"):
                    ...
                    try:
                        ...
                        if "has not been loaded" not in str(e):
                            log_error(f"Cog {file} could not unload. Error: {e}")
                        pass
                    else:
                        pass
            ...
            try:
                ...
                await ctx.respond(
                    f"Error reloading cog `{extension}.py`: {e}", ephemeral=True
                )
    ...
    async def bot_cleartemp(ctx):
        ...
        await ctx.followup.send(
            f"Cleared {count} files from the temp folder.", ephemeral=True
        )

    @bot.event
    async def on_member_join(member):
        log_info(
            f"{member} has joined the guild '{member.guild.name}' [{member.guild.id}]"
        )

    @bot.event
    async def on_member_remove(member):
        log_info(
            f"{member} has left the guild '{member.guild.name}' [{member.guild.id}]"
        )
