...
class Moderation(commands.Cog):
    ...
    async def mod_kick(
        self, ctx, member: discord.Member, reason: str = None, notify: bool = True
    ):
        ...
        try:
            ...
            if dmsent:
                ...
                await ctx.respond(
                    f"Kicked {member.mention} from the server, but could not send a DM to notify them."
                )
        except discord.Forbidden:
    ...
    async def mod_ban(
        ...
        try:
            ...
            if dmsent:
                ...
                await ctx.respond(
                    f"Banned {member.mention} from the server, but could not send a DM to notify them."
                )
        except discord.Forbidden:
    ...
    async def mod_unban(
        self, ctx, user_id: str, reason: str = None, notify: bool = True
    ):
        if not ctx.author.guild_permissions.ban_members:
            raise commands.MissingPermissions(["ban_members"])
        try:
            ...
            if dmsent:
                ...
                await ctx.respond(
                    f"Unbanned {user.mention} from the server, but could not send a DM to notify them."
                )
        except discord.Forbidden:
            raise BotMissingPermissionsError(["ban_members"])

    @mod_group.command(
        name="slowmode", description="Sets the slowmode for the channel."
    )
    async def mod_slowmode(self, ctx, duration: str):
        ...
        try:
            ...
            try:
                ...
                await ctx.respond(
                    "Invalid duration format. Use a number followed by a unit (`10s`, `5m`, `1h`).",
                    ephemeral=True,
                )
        except discord.Forbidden:
    ...
    @mod_group.command(
        name="log_channel", description="Sets the log channel for the server."
    )
    ...
    async def on_message_delete(self, message):
        ...
        embed = discord.Embed(
            title=f"{message.author}'s Message", description=description
        )
        embed.timestamp = datetime.datetime.now()
