...
class Wiki(commands.Cog):
    ...
    @wiki_group.command(
        name="search", description="Searches for a list of Wikipedia articles."
    )
    async def wiki_search(self, ctx, request: str):
        try:
            await ctx.response.defer()
            wikicontent = wikipedia.search(
                request, results=10, suggestion=False
    ...
    @wiki_group.command(
        name="article",
        description="Returns a summary of the Wikipedia article specified.",
    )
    async def wiki_article(self, ctx, request: str):
        try:
            await ctx.response.defer()
            pagecontent = wikipedia.page(
                request, auto_suggest=False, redirect=True, preload=False
            )
            pagetext = wikipedia.summary(
                request, auto_suggest=False, redirect=True, sentences=5
            )
    ...
    @wiki_group.command(
        name="random", description="Returns the summary of a random Wikipedia article."
    )
    async def wiki_random(self, ctx):
        try:
            ...
            if tries > 1:
                embed.set_footer(
                    text=f"Requested by {ctx.author.name} | {tries} attempts",
                    icon_url=ctx.author.avatar.url,
                )
            else:
                embed.set_footer(
                    text=f"Requested by {ctx.author.name} | {tries} attempt",
                    icon_url=ctx.author.avatar.url,
                )
            embed.timestamp = datetime.datetime.now()
            await ctx.respond(embed=embed)
