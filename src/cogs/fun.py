...
class Fun(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if random.random() < 0.0001:
            ...
            embed = discord.Embed(
                title="ATTENTION!!!!",
                description="thanks for your attention <:kirbysmile:1063259570232885310>",
                color=int(str(message.author.color)[1:], 16),
            )
            embed.set_image(
                url="https://media.discordapp.net/attachments/962179885231652966/1063261025232760992/attention-thanks-for-your-attention-trolling-poster-3593367102.jpeg"
            )
            embed.timestamp = datetime.datetime.now()
            embed.set_footer(
                text="this message has a 0.01% chance of appearing!",
                icon_url=message.author.avatar.url,
            )
            log_info(
                f"0.01% message triggered by {message.author.name} in {message.guild.name} ({message.guild.id}) #{message.channel.name} ({message.channel.id})"
            )
